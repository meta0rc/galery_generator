...
$subPasta = "";
$regiao = 'Paraná';
$sitename = 'F9 Comunicação Visual';
...
function retornaKeywords(){
    $paginaAssunto = [
        'Adesivos personalizados',
        'Aluguel de totens',
        'Ambientações personalizadas',
        'Banners personalizados',
        'Comunicação visual',
        'Impressão em lonas',
        'Impressões em látex',
        'Impressões em papelão',
        'Impressões em uv',
        'Impressões para pdv',
        'Letras caixa',
        'Painéis acústicos',
        ...
        'Tapumes para construção',
        'Totens de papelão',
        'Totens personalizados',
    ];
    foreach($paginaAssunto as $assunto){
        $assuntoReplaced = str_replace(' ', '-', tirarAcentos($assunto));
        $assuntoLower = strtolower($assuntoReplaced);
        if(strpos($_SERVER['REQUEST_URI'], $assuntoLower)){
            return $assunto;
        }
    }
}

$regiaoLink = tirarAcentos($regiao);
$focus = strtolower(str_replace(' ', '-', retornaKeywords()));
$focoComtraco = tirarAcentos($focus);
$pageConcat = 'galeria/' . $focoComtraco.'-em-'.str_replace(' ', '-', $regiaoLink);
$paginaGaleria = strtolower($pageConcat);
$paginaGaleriaFoco = retornaKeywords(); 

if($_SERVER['REQUEST_URI'] === '/' . $paginaGaleria) {
...
if (isset($_GET["pagina"])) {
    ...
    if (@file_exists($arquivo)) {
        ...
        if($pagina == "googleSet" || $pagina == $paginaGaleria){
        }else{
            header("Location: /404");
        }
...
if(strpos($_SERVER['REQUEST_URI'], $focoComtraco) && $_SERVER['REQUEST_URI'] !== '/' . $paginaGaleria){
    echo "<a class='galeria-pagina' href=". 'https://' .$_SERVER['HTTP_HOST'] . '/'. $paginaGaleria . ">Confira nossa galeria de " . ucwords(str_replace('-', ' ', $paginaGaleriaFoco)) ."</a>";
}
