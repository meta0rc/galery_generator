<?php 
    ...
    function retornaLinks(){
        $focus = strtolower(str_replace(' ', '-', retornaKeywords()));
        $focoComtraco = tirarAcentos($focus);
        $pages = [''];
        $dir = scandir(__DIR__);
        foreach($dir as $files){
            if(strpos($files, $focoComtraco) === false){
                continue;
            }
    ...
    $keywords = [
        $paginaGaleriaFoco, ' ' . $paginaGaleriaFoco . ' '. $regiao, 
        ' Comunicações visuais',
    ];
    
    $nome = $paginaGaleriaFoco;
    
    $title = 'Serviços de '. $nome . ' ' . ucwords($regiao);

    $content_description = "A empresa " . $sitename . ", é especializada quando se trata de " . $title . ". Atuamos no segmento de ". $segmento . " há muito tempo. Conte com nossos especialistas para " . $nome;
    // Header Recortado

    $file_get_suport = file_get_contents(__DIR__ . '/' . retornaLinks()[1]);

    $file_get_suport = file_get_contents(__DIR__ . '/' . retornaLinks()[1]);
    $inicalHeader = strpos($file_get_suport, '<header>'); 
    $inicialAside = strpos($file_get_suport, '<aside ');
    $headerCrop = substr($file_get_suport, $inicalHeader, $inicialAside);
    $finalCrop = strpos($headerCrop, '</aside>');
    $header = substr($headerCrop, 0, $finalCrop);
 ...
 <html lang="pt-br">
    <head>
        ...
        <meta property="og:description" content="<?php echo $content_description ?>">
    ...
    <main>
        ...
        <ul class="galeria">
            <?php
                foreach($images as $key=>$image) {
                    $focus = strtolower(str_replace(' ', '-', retornaKeywords()));
                    $foco = tirarAcentos($focus); 
                    if(strpos($image, $foco) === false) {
                        continue;
                    }
                    $withoutTraces = str_replace('-', ' ', $image);
                    $imageNome = str_replace('.jpg', '', $withoutTraces) . ' ' . $regiao;
    
                    ?>
                        <li>      
                            <img class="thumb-galeria" src="<?php echo $url.'imagens/'.$image?>" alt="<?php echo $imageNome; ?>" width="200" height="200">   
        ...
        <?php 
                    ...
                    $inicial = strpos($file_get_suport, '<article class="readMore"');
                    $end = strpos($file_get_suport, '<div class="collum-subject"'); 
                    $articleCrop = substr($file_get_suport, $inicial, $end);
    ...
    <div id="modal-galeria" class="hide">
        <div id="modal-container">
            ...
            <div class="footer-modal">
                <p>Faça já uma cotação e saiba mais sobre os serviços de <?php echo $nome; ?></p>
